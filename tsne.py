...
def forward(inputs, target, model, criterion, ids, train=True):
   ...
   # torch.set_grad_enabled(train)
    ...
    output = model(input_vars[0], input_vars[1], input_vars[2], visualized= False) # dist_a, dist_b,  w_x, w_y, w_z,pic
    loss, weights = criterion(*(list(output[0:5]) + [target_var, ids]))
    return output[:2], loss, weights

def forward1(inputs, target, model, criterion, ids, train=True):
    target = target.float().cuda(async=True)
    # volatile
   # torch.set_grad_enabled(train)
    input_vars = [torch.autograd.Variable(inp.cuda(), requires_grad=train)
                          for inp in inputs]
    target_var = torch.autograd.Variable(target, requires_grad=train)
    return model(input_vars[0], input_vars[1], input_vars[2], feature=True)
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('99model.pth.tar')['state_dict'])
    model.eval()
    ...
    if(not have_inds):
        ...
        sample_size = 5000
        topk5 = int(np.ceil(.05 * sample_size))
        for i, x in enumerate(val_dataset):
            print(i)
            if i >= sample_size:
                break
        ...
        print(ind)
        with open('/home/zzhehao/JointAttention/result/tsne/indices.txt', 'w+') as f:
            f.write(str(ind))
    else:        
        with open('/home/zzhehao/JointAttention/result/tsne/indices.txt', 'r') as f:
            ind = f.read()
            ind = list(map(int, ind[1:][:-1].split(',')))
        print(ind)
    ...
    for i in ind:
        ...
        # output, loss, weights = forward(inputs, target, model, criterion, meta['id'], train=False)
        fx,fy,fz, dist_xy, dist_zy = forward1(inputs, target, model, criterion, meta['id'], train=False)
        # x, y = output
        # correct = x < y if target[0] > 0 else y < x 
        correct = dist_xy < dist_zy if target[0] > 0 else dist_zy < dist_xy
        # print(x==dist_xy)
        if True:
            fxs.append(fx.cpu().detach().numpy())
            fys.append(fy.cpu().detach().numpy())
            fzs.append(fy.cpu().detach().numpy())
            num+=1
    fxs = np.concatenate(fxs, axis=0)
    fys = np.concatenate(fys, axis=0)
    fzs = np.concatenate(fzs, axis=0)
    divide=False
    if(divide):
        fs = np.concatenate([fxs, fys, fzs], axis = 0)
        print(fs.shape)
        fs_ = TSNE(n_components=2).fit_transform(fs)
        plt.figure()
        plt.scatter(fs_[:, 0], fs_[:, 1], c=[0]*all+[1]*all+[2]*all)
        plt.legend()
        plt.savefig('1.png')
    else:
        fs = np.concatenate([fxs-fys, fzs-fys], axis = 0)
        print(fs.shape)
        fs_ = TSNE(n_components=2).fit_transform(fs)
        plt.figure()
        plt.scatter(np.clip(fs_[:, 0], -10, 10), np.clip(fs_[:, 1], -10, 10), c=['r']*all+['g']*all, marker='+')
        plt.xlim(-8, 8)
        plt.ylim(-8, 8)
        plt.legend()
        plt.savefig('2.png')
