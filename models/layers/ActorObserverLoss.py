...
from models.layers.DistRatioFocal import DistRatioFocal
from models.layers.BlockGradient import BlockGradient
from models.layers.AttentionDist import AttentionDist
...
class ActorObserverLoss(nn.Module):
    def __init__(self, args):
        ...
        self.loss2 = globals()[args.subloss2]
        self.xstorage = {}
        self.ystorage = {}
        self.zstorage = {}
        self.storage = {}
    ...
    def forward(self, dist_a, dist_b, x, y, z, target, ids):
        ...
        w = x * y * z  # p(theta)  selector output

        # update L
        loss = self.loss.apply(dist_a, dist_b, target, self.margin)  # here is the triplet loss
